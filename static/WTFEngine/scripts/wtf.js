...
var WTF = (function() {
    ...
    */
    ...
    function generate() {
        ...
        var type, text, part, html, idea_html, iter = 0, // Safety mechanism
    ...
    function updateCharacter(data){
        ...
        for( var i in data.skills ){
            var el = '.score.' + data.skills[i][0].replace(' ', '-');
            $(el).text( data.skills[i][1]);
        }
            
        if( data.defining_event !== ''){
            $('.defining span').text(data.defining_event);
            $('.defining').show();
        } else { 
            $('.defining').hide();
        }

        $('.trait span').text( data.personality);
    ...
    function getScore( data, stat){
        var c = {};
        c.stat = data.scores[ stat.toUpperCase()];
        c.bonus = data.bonuses[ stat.toUpperCase()];
        return c;
    }
